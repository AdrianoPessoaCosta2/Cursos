...
public class CarroDAOImpl implements CarroDAO {
    ...
    public void save(Carro carro) {
        String sql = "INSERT INTO `agencias`.`carro` (`nome`, `placa`, compradorid) VALUES (?, ?, ?)";
    ...
    public void delete(Carro carro) {
        ...
        String sql = "DELETE FROM `agencias`.`carro` WHERE `id`= ?";
        try (Connection conn = ConexaoFactory.getConexao();
             PreparedStatement ps = conn.prepareStatement(sql);) {
    ...
    public void update(Carro carro) {
        ...
        String sql = "UPDATE `agencias`.`carro` SET `placa`=?,`nome`=? WHERE `id`=?";
        try (Connection conn = ConexaoFactory.getConexao();
             PreparedStatement ps = conn.prepareStatement(sql);) {
    ...
    public List<Carro> selectAll() {
        String sql = "SELECT id, nome, placa compradorid FROM agencias.carro";
    ...
    public List<Carro> searchByName(String nome) {
        String sql = "SELECT id, nome, placa, compradorid FROM agencias.carro where nome like ?";
        List<Carro> carroList = new ArrayList<>();
        try (Connection conn = ConexaoFactory.getConexao();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, "%" + nome + "%");
